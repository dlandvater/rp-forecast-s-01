...
func queryOneProfile(skuP *SKU) *Profile {
	...
	var weeklyPcnt = make([]float32, 52)
	...
	sSQL := "SELECT row_id,org_id,profile_id,responsible, " +
		"weekly_percent1,weekly_percent2,weekly_percent3,weekly_percent4,weekly_percent5,weekly_percent6,weekly_percent7, " +
		"weekly_percent8,weekly_percent9,weekly_percent10,weekly_percent11,weekly_percent12,weekly_percent13,weekly_percent14, " + "weekly_percent15,weekly_percent16,weekly_percent17,weekly_percent18,weekly_percent19,weekly_percent20,weekly_percent21, " + "weekly_percent22,weekly_percent23,weekly_percent24,weekly_percent25,weekly_percent26,weekly_percent27,weekly_percent28, " + "weekly_percent29,weekly_percent30,weekly_percent31,weekly_percent32,weekly_percent33,weekly_percent34,weekly_percent35, " +
		...
		"FROM profiles WHERE org_id = ? AND profile_id = ? ;"
	...
	for {
		...
		err2 := row.Columns(&profileRowDB.RowId, &profileRowDB.OrgId, &profileRowDB.ProfileId, &profileRowDB.Responsible,
...
func querySkuOneItemOneLocation(itemId string, locationId string) (*SKU, error) {
	...
	sSQL := `SELECT row_id,org_id,item_id,location_id,category_id,profile_id,
       ...
       is_highly_seasonal,season_start_date,season_end_date,cost
		FROM sku WHERE org_id = ? AND item_id = ? AND location_id = ? `
	...
	for {
		...
		err2 := row.Columns(&skuRow.RowId, &skuRow.OrgId, &skuRow.ItemId, &skuRow.LocationId, &skuRow.CategoryId, &skuRow.ProfileId,
...
func querySalesHistoryRows(skuP *SKU) []SalesHistory {
	...
	sSQL := "SELECT row_id, org_id,item_id,location_id,postal_code,start_date,sale_qty,promotion,abnormal_demand,adjusted_sale_qty " +
		"FROM history_sales_weekly " +
		"WHERE org_id = ? AND item_id = ? AND location_id = ? " +
		"ORDER BY start_date ;"
	...
	for {
		...
		if err2 := row.Columns(&nextSalesHistory.RowId, &nextSalesHistory.OrgId, &nextSalesHistory.ItemId,
			...
			&nextSalesHistory.AdjustedQty); err2 != nil {
			insertErrorLog(skuP.OrgId, skuP.ItemId, skuP.LocationId, err, 1)
			return salesHistoryRows
		}
...
func queryForecastBaselineRows(skuP *SKU) []ForecastBaseline {
	...
	sSQL := "SELECT row_id, org_id, item_id, location_id, type, days, start_date, end_date, quantity, sold, selling_price " +
		"FROM forecast " +
		"WHERE org_id = ? AND item_id = ? AND location_id = ? " +
		"ORDER BY start_date ;"
	...
	for {
		...
		if err2 := row.Columns(&nextForecast.RowId, &nextForecast.OrgId, &nextForecast.ItemId, &nextForecast.LocationId,
			&nextForecast.Type, &nextForecast.Days, &nextForecast.StartDate, &nextForecast.EndDate, &nextForecast.Quantity,
			&nextForecast.Sold, &nextForecast.SellingPrice); err2 != nil {
			insertErrorLog(skuP.OrgId, skuP.ItemId, skuP.LocationId, err, 1)
			return forecastRows
		}
...
func queryOneCategory(skuP *SKU) *Category {
	...
	sSQL := `SELECT row_id,org_id,location_id,responsible,profile_id
		FROM category WHERE org_id = ? AND category_id = ? AND location_id = ? `
	...
	for {
		...
		err2 := row.Columns(&categoryRow.RowId, &categoryRow.OrgId, &categoryRow.LocationId, &categoryRow.Responsible,
...
func updateFcstRow(rowId string, skuP *SKU, baseFcstType string, days int,
	startDate time.Time, endDate time.Time, quantity float32, sold float32, sellingPrice sql.NullFloat64) {
...
func insertFcstRow(skuP *SKU, baseFcstType string, days int,
	startDate time.Time, endDate time.Time, quantity float32, sold float32, sellingPrice sql.NullFloat64) string {
